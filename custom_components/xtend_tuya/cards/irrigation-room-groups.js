 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */const dt="_irrigation_timer_registry",pt="Unassigned";function ut(t,e,s){const i=`valve.${t.replace(/^sensor\./,"").replace(new RegExp(`${dt}$`),"")}_valve`;if(e.states[i])return i;const r=`${s} Valve`;for(const t of Object.keys(e.states))if(t.startsWith("valve.")&&e.states[t].attributes?.friendly_name===r)return t;return null}class ft extends nt{setConfig(t){this._config=t}getCardSize(){return 6}shouldUpdate(t){return t.has("_config")||t.has("hass")}_groups(){const t=new Map;for(const e of Object.keys(this.hass.states)){if(!e.endsWith(dt))continue;const s=this.hass.states[e].attributes,i=s.valve_name||s.friendly_name||e,r=i.replace(/\s*Irrigation timer registry$/i,"").trim()||i,o=(s.valve_home||"").trim()||pt,n=(s.valve_room||"").trim()||pt,a=ut(e,this.hass,r),h=a?this.hass.states[a]?.state??"unknown":"unknown";t.has(o)||t.set(o,new Map);const c=t.get(o);c.has(n)||c.set(n,[]),c.get(n).push({name:r,state:h,entity:a})}for(const e of t.values())for(const t of e.values())t.sort((t,e)=>t.name.localeCompare(e.name,void 0,{numeric:!0}));return t}_dotClass(t){return"open"===t||"on"===t?"on":"closed"===t||"off"===t?"off":"na"}_openMoreInfo(t){t&&this.dispatchEvent(new CustomEvent("hass-more-info",{detail:{entityId:t},bubbles:!0,composed:!0}))}render(){if(!this._config||!this.hass)return W;const t=this._groups();if(0===t.size)return B`<ha-card
