 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */function dt(t){return ht({...t,state:!0,attribute:!1})}let pt=class extends ot{constructor(){super(...arguments),this._mode="duration",this._target=60,this._tick=0,this._tickHandle=null}setConfig(t){if(!t.valve)throw new Error("Please define a valve switch entity");if(!t.device_id)throw new Error("Please define a device_id");this._config=t}getCardSize(){return 3}connectedCallback(){super.connectedCallback(),this._tickHandle=window.setInterval(()=>this._tick=Date.now(),1e3)}disconnectedCallback(){super.disconnectedCallback(),null!==this._tickHandle&&(window.clearInterval(this._tickHandle),this._tickHandle=null)}updated(t){t.has("hass")&&this.hass&&this._config&&void 0===t.get("hass")&&this._initTargetFromState()}_initTargetFromState(){if(!this._config.duration)return;const t=this.hass.states[this._config.duration];if(!t)return;const e=parseFloat(t.state);Number.isFinite(e)&&e>0&&(this._target=e)}_isOn(){const t=this.hass.states[this._config.valve];return"on"===t?.state}_activeMode(){if(!this._config.mode_sensor)return null;const t=this.hass.states[this._config.mode_sensor];return t?"duration"===t.state?"duration":"volume"===t.state?"volume":null:null}_targetValue(){if(!this._config.value_sensor)return null;const t=this.hass.states[this._config.value_sensor];if(!t)return null;const e=parseFloat(t.state);return Number.isFinite(e)?e:null}_startTime(){if(!this._config.start_time_sensor)return null;const t=this.hass.states[this._config.start_time_sensor];if(!t||!t.state)return null;const e=new Date(t.state.replace(" ","T"));return Number.isFinite(e.getTime())?e:null}_endTime(){if(!this._config.end_time_sensor)return null;const t=this.hass.states[this._config.end_time_sensor];if(!t||!t.state)return null;const e=new Date(t.state.replace(" ","T"));return Number.isFinite(e.getTime())?e:null}_currentVolume(){if(!this._config.volume_sensor)return null;const t=this.hass.states[this._config.volume_sensor];if(!t)return null;const e=parseFloat(t.state);return Number.isFinite(e)?e:null}_valveName(){if(!this._config.registry_entity)return null;const t=this.hass.states[this._config.registry_entity];return t?.attributes?.valve_name??null}async _toggleManual(){if(!this.hass)return;const t=this._isOn()?"turn_off":"turn_on";await this.hass.callService("switch",t,{entity_id:this._config.valve})}async _startSingleWatering(){this.hass&&(this._config.duration&&"duration"===this._mode&&await this.hass.callService("number","set_value",{entity_id:this._config.duration,value:this._target}),await this.hass.callService("switch","turn_on",{entity_id:this._config.valve}))}async _stop(){this.hass&&await this.hass.callService("switch","turn_off",{entity_id:this._config.valve})}render(){if(!this._config||!this.hass)return B;const t=this._config.name??this._valveName()??this.hass.states[this._config.valve]?.attributes?.friendly_name??"Watering",e=this._isOn(),i=this._startTime(),s=this._endTime(),r=e&&null!==i&&(null===s||i>s);return I`
      <ha-card>
    ...
    `}_renderStatusPill(t,e){return e?I`<span class="pill running">Watering</span>`:t?I`<span class="pill manual">Manual ON</span>`:I`<span class="pill idle">Idle</span>`}_renderProgress(t){this._tick;const e=this._activeMode()??this._mode,i=this._targetValue()??this._target;if("volume"===e){const t=this._currentVolume()??0,e=i>0?Math.min(100,t/i*100):0,s=Math.max(0,i-t);return I`
      ...
      <button class="stop-btn" @click=${this._stop}>Stop</button>
    `}_renderControls(){const t=this._startTime(),e=this._endTime();return I`
      <div class="mode-tabs">
        <button
      ...
      ${t||e?I`
            <div class="last-cycle">
              ${t?I`<div><span class="dim">Last start</span> ${this._fmtDate(t)}</div>`:B}
              ${e?I`<div><span class="dim">Last end</span> ${this._fmtDate(e)}</div>`:B}
            </div>
          `:B}
    `}_setMode(t){this._mode!==t&&("duration"===t&&this._target<5&&(this._target=60),"volume"===t&&this._target>1e3&&(this._target=10),this._mode=t)}_fmtDate(t){return t.toLocaleString(void 0,{hour:"2-digit",minute:"2-digit",day:"2-digit",month:"short"})}};function ut(t){if(!Number.isFinite(t)||t<0)return"0:00";const e=Math.round(t),i=Math.floor(e/3600),s=Math.floor(e%3600/60),r=e%60;return i>0?`${i}:${String(s).padStart(2,"0")}:${String(r).padStart(2,"0")}`:`${s}:${String(r).padStart(2,"0")}`}pt.styles=((t,...e)=>{const i=1===t.length?t[0]:e.reduce((e,i,s)=>e+(t=>{if(!0===t._$cssResult$)return t.cssText;if("number"==typeof t)return t;throw Error("Value passed to 'css' function must be a 'css' function result: "+t+". Use 'unsafeCSS' to pass non-literal values, but take care to ensure page security.")})(i)+t[s+1],t[0]);return new n(i,t,s)})`
    :host {
